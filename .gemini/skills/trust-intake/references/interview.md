...
8. Fill `needed_metrics` (volume, rate, euro_impact, trend, baseline, cx_fp_cost). Rates are fractions (`0.12` for 12%). Those values are the ledger.
...
Then run `memo` and stop. Give the author the printed `approve --confirm` command. Do not run it. After they approve: `render`, `validate`, `decide --run <id>`. You are not done until `decide` exits 0.
